...
Status: Active
Domain: ARCH
Canonical: `docs/ARCH_DOCS_INTERNAL_STRUCTURE_PLAN.md`
Related: `docs/README.md`, `docs/ARCH_DIRECTORY_GOVERNANCE.md`, `docs/ARCH_REFACTOR_TODO.md`, `docs/ARCH_REFACTOR_TASK_PROGRESS.md`
...
当前已完成 Phase 1 和 Phase 2 的物理迁移，根目录保留 Deprecated stub 兼容历史引用。短期重点转为 Phase 3 架构文档迁移准备；新增正式文档必须直接进入目标目录，并在 README 与迁移表中登记。
...
| 目录 | 职责 | 典型文档 |
|---|---|---|
| `docs/` | 总入口、迁移期间的旧路径兼容 stub。 | `README.md`。 |
...
| `docs/archive/` | 已废弃、旧版本或只保留追溯价值的文档。 | deprecated docs、旧迁移 stub。 |
...
|---|---|
| `README.md` | 文档总入口和导航。 |
| 旧路径 stub | 临时兼容历史引用，迁移稳定后归档。 |

根目录不再新增普通主题文档。新增文档应直接进入目标领域目录；如果领域目录尚未实际创建，则先按前缀放在根目录，并在 README 中登记目标目录。
...
当前状态：第一批已迁移 `ENV_`、`MFG_`、`REPORT_`、`TEST_`、`LAYOUT_` 文档到目标目录，旧路径保留 Deprecated stub。
...
当前状态：已迁移 `DATA_`、`FLOW_`、`OPT_`、`RESULT_` 与器件分支文档到 `docs/data/`、`docs/flow/`、`docs/opt/`、`docs/result/`、`docs/devices/`，旧路径保留 Deprecated stub；README、Related 和主文档引用已做第一轮更新。
...
- 对 schema、manifest、state machine 等核心文档保留旧路径 stub 至少一个完整优化周期。
...
| `ADS版图自动仿真项目框架设计.md` | `docs/arch/` | Phase 3 |
| `ARCH_ADS_ASSET_MIGRATION_20260801.md` | `docs/arch/` | Phase 3 |
| `ARCH_ADS_ASSET_MIGRATION_20260801.csv` | `docs/arch/` | Phase 3 |
| `ARCH_DIRECTORY_GOVERNANCE.md` | `docs/arch/` | Phase 3 |
| `ARCH_DOCS_INTERNAL_STRUCTURE_PLAN.md` | `docs/arch/` | Phase 3 |
| `ARCH_FRAMEWORK_REVIEW_GAP_ANALYSIS.md` | `docs/arch/` | Phase 3 |
| `ARCH_REFACTOR_TASK_PROGRESS.md` | `docs/arch/` | Phase 3 |
| `ARCH_REFACTOR_TODO.md` | `docs/arch/` | Phase 3 |
| `PYTHON_SCRIPT_MANAGEMENT.md` | `docs/arch/` | Phase 3 |
...
```text
docs/ARCH_DOCS_MIGRATION_<YYYYMMDD>.csv
```
...
| `new_path` | 迁移后路径。 |
| `doc_domain` | 文档领域。 |
| `status` | moved/stubbed/deprecated/kept。 |
...
Related: `docs/README.md`, `docs/ARCH_DOCS_INTERNAL_STRUCTURE_PLAN.md`
...
- Related 链接不指向不存在文件。
- 项目流程文档和报告模板没有断链。
- 任务记录写入 `ARCH_REFACTOR_TASK_PROGRESS.md`。
...
3. 每批迁移后更新 README、Canonical、Related 和 `ARCH_DOCS_MIGRATION_20260801.csv`，并继续保留旧路径 stub。
4. 对 Phase 1/2 的旧路径 stub 至少保留一个完整优化周期，再进入 Phase 4 清理。
