...
- [ ] 2026-08-06 继续 DUAL 端口修复：当前 `DUAL_END_SMA_CPW_30MM` 已通过 API 加回 connector instance `81/82`，Z=`2.085mm`，并删除旧残留 `79`；明天先串行只读探测 `81/82` 的真实 connector internal excitation/port instance，再按“选中连接器自身激励端口 -> Port > Create -> schematic IPort 连接 connector”复现，禁止再保存 `ConnectionPoints=NONE` 的无箭头 component-pin-only 端口。
