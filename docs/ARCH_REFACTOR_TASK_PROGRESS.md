...
Status: Active
Domain: ARCH
Canonical: `docs/ARCH_REFACTOR_TASK_PROGRESS.md`
Related: `docs/ARCH_REFACTOR_TODO.md`, `docs/ADS版图自动仿真项目框架设计.md`, `docs/PYTHON_SCRIPT_MANAGEMENT.md`
...
- 完成内容：
  ...
  - 迁移 `DATA_`、`FLOW_`、`OPT_`、`RESULT_` 文档到对应目标目录。
  ...
  - 对主文档和项目文档中的旧根目录引用执行第一轮保守替换；迁移 CSV 的 `old_path` 字段保持旧路径不变。
- 验证结果：
...
- 关联文件：
  ...
  - `docs/README.md`
  - `docs/ARCH_DOCS_MIGRATION_20260801.csv`
  - `docs/ARCH_DOCS_INTERNAL_STRUCTURE_PLAN.md`
...
- 关联文件：
  - `docs/ARCH_DOCS_INTERNAL_STRUCTURE_PLAN.md`
  - `docs/ARCH_DOCS_MIGRATION_20260801.csv`
  - `docs/ARCH_DIRECTORY_GOVERNANCE.md`
  - `docs/ARCH_REFACTOR_TODO.md`
  - `docs/README.md`
- 下一步：
...
- 完成内容：
  ...
  - 支持 `gaps_mm[0]`、`gaps_mm[5]` 等索引字段，用于迁移旧脚本中的对称/局部 gap 调整。
  ...
  - 更新 `docs/opt/ROUND_SCRIPT_MIGRATION_PLAN.md`、`docs/README.md`、`docs/PYTHON_SCRIPT_MANAGEMENT.md` 和 `docs/ARCH_REFACTOR_TODO.md`。
- 验证结果：
...
- 关联文件：
  ...
  - `docs/opt/ROUND_SCRIPT_MIGRATION_PLAN.md`
  - `docs/README.md`
  - `docs/PYTHON_SCRIPT_MANAGEMENT.md`
  - `docs/ARCH_REFACTOR_TODO.md`
...
- 完成内容：
  ...
  - 自动扫描 `tools/make_*round*.py` 与 `tools/make_next_filter_candidates.py`，对未登记脚本输出 warning。
  - 更新 `docs/README.md`、`docs/PYTHON_SCRIPT_MANAGEMENT.md` 和 `docs/ARCH_REFACTOR_TODO.md`，将该检查纳入 optimizer/script governance gate。
- 验证结果：
...
- 完成内容：
  ...
  - project loader 对 `generator.script`、`generator.layout_generator`、`optimizer.script`、`optimizer.dataset`、`optimizer.seed_params`、`optimizer.prediction_report` 做 root-relative 路径解析。
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，记录 active sweep optimizer 配置入口已完成，剩余 round 专用脚本迁移仍未完成。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace，未生成新候选文件。
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，将 P2-06 中 sweep 默认值和 profile/project 优先级子项标记完成。
- 验证结果：
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，新增 P2-06 Workflow / Project Config 配置化待办。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace。
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，将 P2-05 editable 安装、版本记录和兼容验证标记为完成。
- 验证结果：
...
- 完成内容：
  ...
  - 输出当前 Python 对应的 editable install 建议命令，但脚本本身不执行安装。
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，新增 P2-05 无副作用检查脚本子项。
  - 更新 `docs/PYTHON_SCRIPT_MANAGEMENT.md`，登记 `check_editable_install.py` 为 host stable smoke 脚本。
- 验证结果：
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，将 P2-04 全部子项标记为完成。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace。
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，将 P2-04 中 `simads.devices` 和 `filter.interdigital` 注册项标记为完成。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace。
  - `python -m py_compile src\simads\devices\__init__.py` 通过。
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，将 P2-03 中 scoring 和 optimizer 模块化子项标记为完成。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace。
...
- 日期：2026-08-01
- 任务目标：
  - 对 P2-02 主流程旧 ADS CLI 的薄转接做集中验证。
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`：主流程旧 ADS CLI 的纯路径/计划/解析 helper 已转接到 `simads.ads`。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace。
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md` 中 P2-02 转接状态。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace。
...
- 关联文件：
  ...
  - `docs/ARCH_REFACTOR_TASK_PROGRESS.md`
- 下一步：
  - 对 P2-02 做一次集中 py_compile/smoke；随后可进入 P2-03 scoring/optimizer 抽象，或先补 ADS Python profile/API smoke。
...
- 还需完成：
  ...
  - 后续可让导入流程优先读取 `_layout.json`，减少对 DXF 反解析的依赖。
- 关联文件：
  - `tools/ads_import_dxf_add_ports.py`
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md` 中 P2-02 骨架状态，并保留旧 CLI 转接和 ADS Python smoke 后续项。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace。
  ...
  - 对照现有旧 CLI 参数后，已将 emSetup 参数保持为 `--points-text`，RFPro 参数保持为 `--start/--stop/--out`。
- 还需完成：
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，P2-01 已完成。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace。
...
- 还需完成：
  - P2-01 已收口；legacy DXF writer 尚未替换为 common DXF writer，后续需要 golden/hash 对比后再做。
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md` 中 P2-01 迁移状态。
- 验证结果：
  - 本任务未启动 ADS/FEM，未写入外部 ADS workspace。
...
- 日期：2026-08-01
- 任务目标：
  - 推进 P2-01，将当前主线使用价值较高的交指滤波器 layout generator 接入通用几何对象。
...
- 还需完成：
  ...
  - 后续可建立 legacy DXF 与 common DXF 的 golden/hash 对比，再决定是否统一替换 DXF writer。
- 关联文件：
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md` 中 P2-01 前两项。
- 验证结果：
...
- 完成内容：
  ...
  - 更新 `docs/README.md` 和 `docs/ARCH_REFACTOR_TODO.md`。
- 验证结果：
...
- 任务目标：
  ...
  - 记录人工操作的时间、对象、动作、原因、截图/导出文件和复现影响。
- 完成内容：
...
- 完成内容：
  ...
  - 定义制造和材料扰动变量：Er、tanD、介质厚度、铜厚、线宽、间距、长度、过渡、via 和对位。
...
- 完成内容：
  ...
  - 读取现有 `i7_fr4_baseline_freeze_20260801.md/json`，将已冻结对象、漂移容差和 legacy run 说明纳入通用规则。
...
- 验证结果：
  ...
  - P1-05 TODO 已勾选。
- 还需完成：
  - 后续 P2 可实现 baseline repeat 的自动比对脚本，读取 frozen JSON 并输出 `repeat_pass/repeat_warn/repeat_drifted/repeat_invalid`。
...
- 完成内容：
  ...
  - 对照 `tools/analyze_ads_dataset.py`，记录当前硬约束、回损目标、margin 公式和 `PASS_CANDIDATE` / `TUNE` 判定。
...
- 完成内容：
  ...
  - 更新 `docs/README.md`、`docs/data/DATA_RUN_MANIFEST_SCHEMA.md` 和 `docs/ARCH_REFACTOR_TODO.md`。
- 验证结果：
...
- 完成内容：
  ...
  - 更新 `docs/README.md`、`docs/ARCH_REFACTOR_TODO.md` 和 `docs/data/DATA_RUN_MANIFEST_SCHEMA.md`。
- 验证结果：
...
- 完成内容：
  - 新增 `docs/ARCH_DIRECTORY_GOVERNANCE.md`。
...
- 完成内容：
  ...
  - 对成功候选，summary 从 manifest/state 合并 `run_id/project_id/round_id/candidate_id/profile_id/target_profile_id/score_version/elapsed_s`。
  - 对失败候选，summary 保留预生成 `run_id/run_dir`，并保留失败分类字段。
...
- 完成内容：
  ...
  - 更新 `docs/ARCH_REFACTOR_TODO.md`，将 P0-02/P0-03 文档项标记为完成，并保留 P0-04/P0-05 代码回填任务。
...
- 完成内容：
  ...
  - 新增 `docs/ARCH_ADS_ASSET_MIGRATION_20260801.md` 和 `docs/ARCH_ADS_ASSET_MIGRATION_20260801.csv`。
  - 更新 `docs/README.md`、`docs/ARCH_REFACTOR_TODO.md`、`docs/ADS版图自动仿真项目框架设计.md` 中的当前边界。
- 验证结果：
  - 迁移索引生成 199 条记录。
...
- 关联文件：
  ...
  - `ADS/README.md`
  - `docs/ARCH_ADS_ASSET_MIGRATION_20260801.md`
  - `docs/ARCH_ADS_ASSET_MIGRATION_20260801.csv`
  - `tools/build_i7_fr4_optimization_dataset.py`
  - `tools/propose_i7_fr4_surrogate_candidates.py`
- 下一步：
...
- 完成内容：
  ...
  - 更新 `docs/PYTHON_SCRIPT_MANAGEMENT.md`，登记第一批重构落地项。
- 验证结果：
