...
本文档定义 HFSS 独立扩展项：用一段标准 50 ohm 微带线和两端连接器组成联合仿真模型，通过 HFSS 自动化迭代连接器与微带 launch 处的版图参数，使微带-连接器过渡的 S 参数表现更优。该流程不引入滤波器、谐振器或其他功能结构，目的是降低系统复杂度，把优化对象集中在连接器焊盘、锥形过渡、地过孔、参考地和端口参考面。

## 1. 目标

当前目标是微带线+连接器联合仿真，不优化滤波器主体，也不把滤波器接入模型。模型只包含：P1 连接器 launch、一段标准 50 ohm 微带线、P2 连接器 launch。通过这个独立联合仿真模型判断连接器与微带过渡区域对回波、插损、相位和左右端一致性的影响。

核心目标：

- 固定层叠、50 ohm 线宽、直通线长度和求解频段，减少非连接器变量。
- 参数化 P1/P2 两端连接器过渡区域。
- 自动生成 microstrip+connector layout JSON 或 HFSS 几何输入。
- 用 HFSS 作为主要求解器，输出包含连接器/launch 影响的 S2P、trace CSV、score CSV、SVG 和 manifest。
- 以连接处参数为主变量进行迭代，优化 4-10 GHz 范围内的 S11/S22、S21 插损、端口对称性和 6-8 GHz 目标频段表现。
- 将理想 50 ohm through line 或无连接器 50R 微带线作为 baseline，用 delta 指标判断连接器结构是否引入额外劣化。
