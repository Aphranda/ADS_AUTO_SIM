#!/usr/bin/env python3
"""Plot connector launch S-parameter curves as SVG files."""
...
def decorate_axes(ax: plt.Axes, title: str, x_min: float, x_max: float) -> None:
    ...
    ax.axhline(-20.0, color="#8a8f98", alpha=0.35, linewidth=0.8, linestyle=":")
    ax.set_title(title)
...
def plot_overlay(
    ...
    for candidate, path in selected:
        ...
        ax.plot(freq, traces["s21"], label=candidate, linewidth=1.1, alpha=0.82)
    x_low, x_high = resolve_x_limits(all_freq, x_min, x_max)
    decorate_axes(ax, f"{label} S21 overlay", x_low, x_high)
    if show_legend:
        ax.legend(loc="center left", bbox_to_anchor=(1.01, 0.5), fontsize=6)
    out = out_dir / f"{label}_s21_overlay.svg"
    fig.savefig(out, format="svg", bbox_inches="tight")
    plt.close(fig)
    return out
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate connector SVG S-parameter curves from trace CSV results.")
    parser.add_argument("--summary", type=Path, required=True)
    parser.add_argument("--out-dir", type=Path, required=True)
    parser.add_argument("--candidate", action="append", default=None, help="Candidate to plot. May be repeated.")
    parser.add_argument("--sparams", default="s11,s21,s22", help="Comma-separated list: s11,s21,s22.")
    ...
    parser.add_argument("--overlay-label", default="connector")
    parser.add_argument(
        "--individual-legend",
...
def main() -> None:
    ...
    sparams = [item.strip().lower() for item in args.sparams.split(",") if item.strip()]
    if not args.no_individual:
        for candidate, path in sources:
            plot_individual(candidate, path, out_dir, sparams, args.x_min, args.x_max, args.individual_legend)
    if not args.no_overlay and len(sources) > 1:
        plot_overlay(sources, out_dir, args.overlay_max, args.overlay_label, args.x_min, args.x_max, args.overlay_legend)
