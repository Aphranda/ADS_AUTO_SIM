#!/usr/bin/env python3
"""Generate the next 7th-order FR4 interdigital candidate set.

This round keeps the current best geometry as the center point and adds
small joint sweeps on tap, taper length, taper tip width, and outer gaps.
The intent is to push S11/S22 down without sacrificing the current 5 GHz
rejection and 8 GHz edge response.
"""

from __future__ import annotations

import argparse
import csv
import json
from dataclasses import replace
from pathlib import Path

from generate_interdigital_filter_layout import FilterParams, write_outputs


FIELDNAMES = [
    "name",
    "L_mm",
    "tap_mm",
    "Egap_mm",
    "S1_mm",
    "S2_mm",
    "S3_mm",
    "S4_mm",
    "S5_mm",
    "S6_mm",
    "feed_taper_len_mm",
    "feed_tip_w_mm",
    "feed_overlap_mm",
    "via_diameter_mm",
    "metal_layer",
    "via_layer",
    "notes",
]


def fmt(value: float) -> str:
    return f"{value:.4f}".rstrip("0").rstrip(".")


def load_seed(path: Path) -> FilterParams:
    data = json.loads(path.read_text(encoding="utf-8"))
    params = data["parameters"]
    return FilterParams(
        name=str(params["name"]),
        order=int(params["order"]),
        substrate=str(params["substrate"]),
        er=float(params["er"]),
        dielectric_height_mm=float(params["dielectric_height_mm"]),
        copper_thickness_mm=float(params["copper_thickness_mm"]),
        lower_cutoff_ghz=float(params["lower_cutoff_ghz"]),
        upper_cutoff_ghz=float(params["upper_cutoff_ghz"]),
        passband_ripple_db=float(params["passband_ripple_db"]),
        z0_ohm=float(params["z0_ohm"]),
        w0_mm=float(params["w0_mm"]),
        resonator_w_mm=float(params["resonator_w_mm"]),
        resonator_l_mm=float(params["resonator_l_mm"]),
        tap_from_bottom_mm=float(params["tap_from_bottom_mm"]),
        end_gap_mm=float(params["end_gap_mm"]),
        gaps_mm=tuple(float(value) for value in params["gaps_mm"]),
        feed_len_mm=float(params["feed_len_mm"]),
        feed_taper_len_mm=float(params["feed_taper_len_mm"]),
        feed_tip_w_mm=float(params["feed_tip_w_mm"]),
        feed_overlap_mm=float(params["feed_overlap_mm"]),
        boundary_margin_mm=float(params["boundary_margin_mm"]),
        min_fab_feature_mm=float(params["min_fab_feature_mm"]),
        metal_layer=str(params["metal_layer"]),
        via_layer=str(params["via_layer"]),
        via_diameter_mm=float(params["via_diameter_mm"]),
        via_pad_mm=float(params["via_pad_mm"]),
        via_half_outside=bool(params["via_half_outside"]),
        via_pad_outside=bool(params["via_pad_outside"]),
    )


def row_from_params(params: FilterParams, notes: str) -> dict[str, str]:
    gaps = list(params.gaps_mm)
    if len(gaps) != 6:
        raise ValueError("expected 6 gaps for the 7th-order layout")
    return {
        "name": params.name,
        "L_mm": fmt(params.resonator_l_mm),
        "tap_mm": fmt(params.tap_from_bottom_mm),
        "Egap_mm": fmt(params.end_gap_mm),
        "S1_mm": fmt(gaps[0]),
        "S2_mm": fmt(gaps[1]),
        "S3_mm": fmt(gaps[2]),
        "S4_mm": fmt(gaps[3]),
        "S5_mm": fmt(gaps[4]),
        "S6_mm": fmt(gaps[5]),
        "feed_taper_len_mm": fmt(params.feed_taper_len_mm),
        "feed_tip_w_mm": fmt(params.feed_tip_w_mm),
        "feed_overlap_mm": fmt(params.feed_overlap_mm),
        "via_diameter_mm": fmt(params.via_diameter_mm),
        "metal_layer": params.metal_layer,
        "via_layer": params.via_layer,
        "notes": notes,
    }


def make_variant(seed: FilterParams, name: str, notes: str, **updates: object) -> tuple[FilterParams, dict[str, str]]:
    params = replace(seed, name=name, **updates)
    return params, row_from_params(params, notes)


def edge_gap_variant(seed: FilterParams, delta: float) -> tuple[FilterParams, dict[str, str]]:
    gaps = list(seed.gaps_mm)
    min_feature = seed.min_fab_feature_mm
    gaps[0] = max(min_feature, round(gaps[0] + delta, 4))
    gaps[5] = max(min_feature, round(gaps[5] + delta, 4))
    params = replace(seed, name=f"i7_fr4_r2_s1{('p' if delta > 0 else 'm')}_s6{('p' if delta > 0 else 'm')}", gaps_mm=tuple(gaps))
    note = "relax outer coupling to test match recovery" if delta > 0 else "tighten outer coupling to check rejection / match tradeoff"
    return params, row_from_params(params, note)


def build_round2(seed: FilterParams) -> list[tuple[FilterParams, dict[str, str]]]:
    base = seed
    rows: list[tuple[FilterParams, dict[str, str]]] = []

    rows.append(make_variant(base, "i7_fr4_r2_base", "repeat current best point as the baseline"))
    rows.append(make_variant(base, "i7_fr4_r2_t190", "lower tap to see if return loss improves", tap_from_bottom_mm=1.90))
    rows.append(make_variant(base, "i7_fr4_r2_t195", "keep nominal tap as a control", tap_from_bottom_mm=1.95))
    rows.append(make_variant(base, "i7_fr4_r2_t200", "raise tap slightly to compare match and edge loss", tap_from_bottom_mm=2.00))
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t190_tl045",
            "shorter taper with lower tap to tighten the feed transition",
            tap_from_bottom_mm=1.90,
            feed_taper_len_mm=0.45,
        )
    )
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t190_tl075",
            "longer taper with lower tap to smooth the feed transition",
            tap_from_bottom_mm=1.90,
            feed_taper_len_mm=0.75,
        )
    )
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t190_tw016",
            "narrower taper tip to test stronger local coupling",
            tap_from_bottom_mm=1.90,
            feed_tip_w_mm=0.16,
        )
    )
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t190_tw022",
            "wider taper tip to test weaker local coupling",
            tap_from_bottom_mm=1.90,
            feed_tip_w_mm=0.22,
        )
    )
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t190_s1p",
            "lower tap with relaxed outer coupling",
            tap_from_bottom_mm=1.90,
            gaps_mm=(0.1276, 0.175, 0.1857, 0.1857, 0.175, 0.1276),
        )
    )
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t190_s1m",
            "lower tap with tighter outer coupling",
            tap_from_bottom_mm=1.90,
            gaps_mm=(0.1076, 0.175, 0.1857, 0.1857, 0.175, 0.1076),
        )
    )
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t200_tl075_tw016",
            "aggressive smooth-feed variant to search a lower return-loss pocket",
            tap_from_bottom_mm=2.00,
            feed_taper_len_mm=0.75,
            feed_tip_w_mm=0.16,
        )
    )
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t200_tl045_tw022",
            "compact feed variant to compare against the smoother transition",
            tap_from_bottom_mm=2.00,
            feed_taper_len_mm=0.45,
            feed_tip_w_mm=0.22,
        )
    )
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t195_s1p",
            "nominal tap with relaxed outer coupling",
            tap_from_bottom_mm=1.95,
            gaps_mm=(0.1276, 0.175, 0.1857, 0.1857, 0.175, 0.1276),
        )
    )
    rows.append(
        make_variant(
            base,
            "i7_fr4_r2_t195_s1m",
            "nominal tap with tighter outer coupling",
            tap_from_bottom_mm=1.95,
            gaps_mm=(0.1076, 0.175, 0.1857, 0.1857, 0.175, 0.1076),
        )
    )
    return rows


def parse_args() -> argparse.Namespace:
    root = Path(__file__).resolve().parents[2]
    parser = argparse.ArgumentParser(description="Generate the next FR4 7th-order interdigital candidate set.")
    parser.add_argument(
        "--seed",
        type=Path,
        default=root / "SIM" / "ADS" / "interdigital_7o_fr4_210um_round1" / "i7_fr4_r1_l555_taper_params.json",
    )
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=root / "SIM" / "ADS" / "interdigital_7o_fr4_210um_round2",
    )
    parser.add_argument(
        "--plan",
        type=Path,
        default=root / "SIM" / "ADS" / "filter_opt_i7_fr4_round2.csv",
    )
    return parser.parse_args()


def main() -> None:
    args = parse_args()
    seed = load_seed(args.seed)
    rows = build_round2(seed)

    args.out_dir.mkdir(parents=True, exist_ok=True)
    plan_rows: list[dict[str, str]] = []
    print(f"Generating {len(rows)} candidates into {args.out_dir}")
    for params, row in rows:
        outputs = write_outputs(params, args.out_dir)
        plan_rows.append(row)
        print(f"  {params.name}: {outputs['dxf_mm_coords']}")

    args.plan.parent.mkdir(parents=True, exist_ok=True)
    with args.plan.open("w", newline="", encoding="utf-8") as fp:
        writer = csv.DictWriter(fp, fieldnames=FIELDNAMES)
        writer.writeheader()
        writer.writerows(plan_rows)

    print(f"Wrote plan: {args.plan}")


if __name__ == "__main__":
    main()
